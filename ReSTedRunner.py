...
from ReSTedSyncReSTMenu import Ui_MainWindow
...
dl {padding: 5pt; }
dt {font-variant: small-caps; font-weight: bold; font-size: 14pt; 
    margin-bottom: 5pt; }
...
#        self.ui.actionRedo.triggered.connect(self.undoRedoHistory.redo)
        ...
        # # ReST menu:
        self.ui.actionBulleted\
         .triggered.connect(lambda: self.restList(self.bulet))
        self.ui.actionEnumerated\
         .triggered.connect(lambda: self.restList(self.number))
        self.ui.actionDefinition\
         .triggered.connect(lambda: self.restList(self.definition))
        self.ui.actionField\
         .triggered.connect(lambda: self.notYI(self.ui.actionField))
        self.ui.actionOption\
         .triggered.connect(lambda: self.notYI(self.ui.actionField))
...
"""
    ...
    def newText(self):
        ...
        if self.FLAGS.RSTMODIFIED:
            ...
            resultWrn = QMessageBox.Discard
        if resultWrn == QMessageBox.Discard:
            self.textEdit.clear()
            self.ReSTTitle = self.newFileName
            self.MainWindow.setWindowTitle(self.ReSTedTitle
                                           + ": " + self.ReSTTitle)
    ...
    def loadReST(self, path=None):
        ...
        if not path:
            if self.FLAGS.RSTMODIFIED:
                ...
                resultWrn = QMessageBox.Ok
            if resultWrn == QMessageBox.Ok:
                result = QFileDialog.\
...
#                                   f" {path}")
        ...
        if loadedFile:
            ...
            # searching for css file name:
            match = re.search(r".. +style: +%.+%", loadedFile)
            if match:
                cssFile = re.search(r"%.+%", match.group()).group()[1:-1]
                last = list(re.finditer("/", self.currentFilePath))[-1].end()
                currentPath = self.currentFilePath[:last]
                self.loadCSS(currentPath + cssFile)
                print(f"Attempt to load css file: {currentPath + cssFile}")
#            self.loadCSS("/media/vault/docs/softdev/python/pyqt/projs/rested/html/firstSaved.css")
        else:
...
#        ret = msgBox.exec()
        print(f"{QMessageBox.Ok = }")
        print(f"{QMessageBox.Discard = }")
        return resultWrn
...
#        printd("The text in editor has changed!")
    ...
    # %%
    def restList(self, function):
        """ Transformation: selected text info a list 
        of type defined by `function`. """
    ...
    def definition(self, text):
        """ `definition` list -- first line as the term name. """
        lines = text.split("\n")
        if lines[-1] != '\n':
            lines.append('\n')
        result = lines[0] + "\n"
...
#                          indent=4)
    ...
    def exitApp(self):
        ...
        if response == QMessageBox.Ok or response == QMessageBox.Discard:
            self.CLOSEEVTITER += 1
...
<p><strong>ReSTed</strong> -- plain text editor and""" +\
    f""" restructured text compiler, ver. {version}.</p>
